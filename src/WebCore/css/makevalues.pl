...
my @NAMES = applyPreprocessor($input, $defines, $preprocessor);
...
open GPERF, ">$output" || die "Could not open CSSValueKeywords.gperf for writing";
print GPERF << "EOF";
%{
...
# system("gperf --key-positions=\"*\" -D -n -s 2 $input > $output") == 0 || die "calling gperf failed: $?";
