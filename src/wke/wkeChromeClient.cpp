...
WebCore::IntRect ToolTip::paint(void* bits, int pitch)
{
    ...
    if (!m_pixels)
    {
        ...
        ::DrawText(m_hdc.get(), (LPCWSTR)m_title.characters(), m_title.length(), &rcText, DT_WORDBREAK|DT_CALCRECT);
        ...
        ::DrawText(m_hdc.get(), (LPCWSTR)m_title.characters(), m_title.length(), &rcText, DT_WORDBREAK);
