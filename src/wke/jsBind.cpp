...
const wchar_t* wkeJSToTempStringW(wkeJSState* es, wkeJSValue v)
{
    ...
    s_sharedStringBufferW.assign((wchar_t*)str.characters(), str.length());
...
{
    JSC::JSValue value = JSC::jsString((JSC::ExecState*)es, JSC::UString((UChar*)str));
    return (wkeJSValue)JSC::JSValue::encode(value);
}
...
{
    String s = String::fromUTF8(str);
    return (wkeJSValue)wkeJSEvalW(es, (wchar_t*)s.charactersWithNullTermination());
}
...
wkeJSValue wkeJSEvalW(wkeJSState* es, const wchar_t* str)
{
    ...
    JSC::SourceCode source = JSC::makeSource((UChar*)str);
